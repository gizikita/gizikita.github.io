      ...
      <div className="mt-6 rounded-xl border p-5" style={containerStyle}>
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          ...
          <button onClick={() => downloadCSV([
            { name: 'Contoh Nama', gender: 'L', birthDate: '2023-01-15', measurementDate: '2026-07-01', weight: 8.5, height: 65.0, status: '', parentName: 'Orang Tua', address: 'Alamat' }
    ...
    <div className="mt-6 rounded-xl border p-5" style={containerStyle}>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          {/* Import button — inline with heading */}
          <button onClick={() => fileInputRef.current?.click()}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-medium border transition-colors whitespace-nowrap"
            style={{
      ...
      {confirmDeleteAll && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}>
          <div className="rounded-2xl p-6 max-w-sm w-full shadow-xl border"
            ...
            <p className="text-sm mb-5" style={{ color: 'var(--md-on-surface-variant)' }}>{sorted.length} data akan dihapus dan tidak dapat dikembalikan.</p>
